...
# estimate page 1 table, that includes the first row
page_1_width <- tableGrob(d = stop_times_0_wide[1:30,c(-1,-3)][,1:15], 
                          rows = NULL, 
                          theme = ttheme_default(base_size = 8))

page_1_width <- grid::convertWidth(x = sum(page_1_width$widths), 
                                   unitTo = "inches", 
                                   valueOnly = T)

# estimate the width of all other pages that dont include column 1
page_2_width <- tableGrob(d = stop_times_0_wide[1:30,c(-1,-3)][,16:33], 
                          rows = NULL, 
                          theme = ttheme_default(base_size = 8))

page_2_width <- grid::convertWidth(x = sum(page_2_width$widths), 
                                   unitTo = "inches", 
                                   valueOnly = T)

# estimate the nrows to fit our height parameter
page_height <- tableGrob(d = stop_times_0_wide[1:32,c(-1,-3)][,1:15], 
                          rows = NULL, 
                          theme = ttheme_default(base_size = 8))

page_height <- grid::convertWidth(x = sum(page_height$heights), 
                                   unitTo = "inches", 
                                   valueOnly = T)


# calculate the number of pages needed to fit all trips
ncol_page1 <- 




pdf("table.pdf", width = 11, height = 8.5)
lapply(1:7, FUN = function(x) {
  start_col <- (x*16)-15
  end_col <- x*16
  if (end_col >= (length(stop_times_0_wide)-2)) {
    end_col <- length(stop_times_0_wide)-2
  }
  grid.arrange(
  tableGrob(d = stop_times_0_wide[1:32,c(-1,-3)][,c(start_col:end_col), with=F], 
            rows = NULL, 
            theme = ttheme_default(base_size = 8))
)
})
...
grid.arrange(
  tableGrob(d = stop_times_0_wide[1:32,c(-1,-3)][,c(1:15)], 
            rows = NULL, 
            theme = ttheme_default(base_size = 8))
)
grid.arrange(
  tableGrob(d = stop_times_0_wide[1:32,c(-1,-3)][,c(16:30)], 
            rows = NULL, 
            theme = ttheme_default(base_size = 8))
)
grid.arrange(
  tableGrob(d = stop_times_0_wide[1:32,c(-1,-3)][,c(33:45)], 
            rows = NULL, 
            theme = ttheme_default(base_size = 8))
)
grid.arrange(
  tableGrob(d = stop_times_0_wide[1:32,c(-1,-3)][,c(46:45)], 
            rows = NULL, 
            theme = ttheme_default(base_size = 8))
)
grid.arrange(
  tableGrob(d = stop_times_0_wide[c(1,33:63),c(-1,-3)][,c(1:15)], 
            rows = NULL, 
            theme = ttheme_default(base_size = 8))
)
dev.off()
...
\<iframe src="[https://drive.google.com/file/d/123-baOzQFitfQQ_W2sdP3K0EZYG6Vvh3/preview"](https://drive.google.com/file/d/123-baOzQFitfQQ_W2sdP3K0EZYG6Vvh3/preview") width="640" height="480" allow="autoplay"\>\</iframe\>
