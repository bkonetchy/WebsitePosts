...
# run checks on columns
row_checks <- apply(X = stop_times_0_wide[,c(-1,-2,-3)], MARGIN = 1, FUN = row_check_fun)

# see if all values are true or not
all(row_checks %>% unlist())

# find trip numbers that are the issue
lapply(row_checks, function(x) {which(x != T)})



# we have false values so lets extrac the trips with an error and perform a more indepth analysis 
row_errors <- which(row_checks == F)

# go through each trip and return the lead stop(s) that is causing the issue
row_errors <- lapply(row_errors, FUN = function(x) {
