...
# clean colnames
colnames(stop_times_0_wide) <- c("Stop Order", "Stop Names", 
                            paste0("Trip_", 1:(length(stop_times_0_wide)-2)))

# create table for direction 0 by casting to wide format
stop_times_1_wide <- data.table::dcast.data.table(data = stop_times_1, formula = stop_order+stop_name ~ trip_order, value.var = "arrival_time")
# clean colnames
colnames(stop_times_1_wide) <- c("Stop Order", "Stop Names", 
                            paste0("Trip_", 1:(length(stop_times_1_wide)-2)))
...
col_checks <- apply(X = stop_times_0_wide[,c(-1,-2)], MARGIN = 2, FUN = column_check_fun)
